...
public class SamplePlayerController : OVRPlayerController
{
	...
	//public long importPhoneTimestamp;
	static private float moveSmooth=0.0f; //cms
	public float moveSmoothFactor = 0.5f; //cms
	private float tempPhoneVal; //cms
	...
	public Vector3 lVelocity = Vector3.zero; //cms
	public Vector3 rVelocity = Vector3.zero; //cms
	//public Text debugText; //cms 
	public Text Velocity; //cms
    ...
    public override void UpdateMovement()
    {
		...
		/* as we rotate the HMD, the direction for the thumb sticks changes, but the velocity 
		axis does not.  Need to update this */


		//moving using controllers
		lVelocity = OVRInput.GetLocalControllerVelocity (OVRInput.Controller.LTouch);
		rVelocity = OVRInput.GetLocalControllerVelocity (OVRInput.Controller.RTouch);

		/* this seems to do what we want, but if the player is rotated with the thumbsticks it doesn't work.
		 * without this, rotation by thumbsticks works but with head doesn't */
		/* is this desirable? don't want to keep spinning around, can use thumbsticks to turn and head to just look around? */
	//	lVelocity = Quaternion.Euler(playerDirection.eulerAngles) * lVelocity;
	//	rVelocity = Quaternion.Euler(playerDirection.eulerAngles) * rVelocity;
	
		/* with this enabled, the direction is changed by the thumb sticks */
		//	lVelocity = Quaternion.Euler(euler) * lVelocity;
	//	rVelocity = Quaternion.Euler(euler) * rVelocity;
	//	Debug.Log("My phone time: " + NetworkServer.timestampPhoneSensor);

		if(lVelocity.z > 0.3f)
			leftAxisY = lVelocity.z;
		if(rVelocity.z > 0.3f)
			leftAxisY = rVelocity.z; 

		//end moving with controllers

		tempPhoneVal = NetworkServer.zPhoneSensor;
	//	Debug.Log ("My phone Z: " + tempPhoneVal);
		if ((tempPhoneVal > 1.0f)) {
			//Debug.Log (" True: " + tempPhoneVal);


			//leftAxisY = tempPhoneVal;
			moveSmooth = tempPhoneVal;
		} else if (tempPhoneVal < -1.0f) {
		//	leftAxisY = -tempPhoneVal;
			moveSmooth = -tempPhoneVal;
		} else {
			/* if we don't detect movement on z, perhaps there is movement on y? */
			tempPhoneVal = NetworkServer.yPhoneSensor;
			if ((tempPhoneVal > 1.0f)) {
				//Debug.Log (" True: " + tempPhoneVal);
				//	leftAxisY = tempPhoneVal; 
				moveSmooth = tempPhoneVal;
				...
				moveSmooth = moveSmooth * 0.9f;
			}
		}
		if (moveSmooth > 0.3f)
			leftAxisY = moveSmooth;
	//	else if (moveSmooth < -0.3f)
	//		leftAxisY = -moveSmooth;
		...
		/* cms get velocity so that we can tell if user is swinging arms in walking motion */
		//		Velocity.text = activeController.ToString ();
...
//		if (activeController == OVRInput.Controller.LTouch) {
	...
	//	}
		...
		//	Velocity.text = "Right";
		//}
	//	Velocity.text = "lVelocity = <" + lVelocity.x.ToString() + " , " + lVelocity.y.ToString() + " , " + lVelocity.z.ToString() + ">" + "rVelocity = <" + rVelocity.x.ToString() + " , " + rVelocity.y.ToString() + " , " + rVelocity.z.ToString() + ">";
		/* end cms */
